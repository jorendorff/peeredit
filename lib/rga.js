...
RGA.tieToAceEditor = function tieToAceEditor(rga, editor) {
  // It's ok to use the RGA id *if* no other edit originate at rga except edits
  // made by the editor, because we immediately apply any changes that happen
  // in the editorReplica to rga, with no queueing.  This is pretty sketchy,
  // admittedly.
  var editorReplica = new RGA(rga.id, rga.history(), RGA._noQueue);
  ...
  function applyOpToEditor(op) {
    switch (op.type) {
    case "addRight":
      var loc = rga.getRowColumnAfter(op.t, op.w.timestamp);

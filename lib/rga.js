...
RGA.prototype = {
  ...
  // Convenience method to apply a patch. The structure of `delta` is the same as a Quill delta, just
  // because it was a JSON patch format I knew about -- RGA doesn't actually
  // use any Quill code.
...
RGA.tieToAceEditor = function tieToAceEditor(rga, editor) {
  ...
  function assertInSync(infodump) {
    ...
    if (lastText != erText) {
      ...
      console.error("lastText and editorReplica are out of sync", infodump);
    }
  }
